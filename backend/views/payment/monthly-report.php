			...
			<div class=" col-md-4" style="padding: 10px;height:auto !important;">
				<table class="table table-reponsive table-striped table-borderd tbale-hover" value="<?PHP echo $id = $val->id;?>" id="<?PHP echo $account_name =  $val->account_name;?>">
					...
					<tbody>
						...
						{
							$date = $_POST['date'];
							$debit_transactions = Transactions::find()->where(['debit_account' => $id])->andwhere(['between' , 'date', $monthstart , $monthend])->andwhere(['organization_id' => \Yii::$app->user->identity->organization_id])->all();
							$credit_transactions = Transactions::find()->where(['credit_account' => $id])->andwhere(['between' , 'date', $monthstart , $monthend])->andwhere(['organization_id' => \Yii::$app->user->identity->organization_id])->all();

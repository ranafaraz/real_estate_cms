...
if(isset($_POST['account_payable_id']) && isset($_POST['s_date']) && isset($_POST['e_date']))
{
	$account_payable_id = $_POST['account_payable_id'];
	...
	$header = Organization::find()->where(['id' => \Yii::$app->user->identity->organization_id])->One();
			...
			$returndata.='<div class="row" style="margin:0px;"><div class="col-md-12"><h4 style="margin-left:10px;">Account Payable Info</h4></div></div><table style="margin-top:10px !important" class="table table-responsive tbale-bordered table-striped">
						<tr class="text-center">
							...
							<th class="text-center text-dark bg-primary">Account Payable</th>
							<th class="text-center text-dark bg-primary">Due Date</th>
							<th class="text-center text-dark bg-primary">Created At</th>
						</tr>';
				$value = AccountPayable::find()->where(['account_payable' => $account_payable_id])->andwhere(['organization_id' => \Yii::$app->user->identity->organization_id])->One();
				if(empty($value))
				{
					$returndata.= "<tr><th class='text-danger text-center' colspan='6'>Sorry! No Record Found From Account Payable!</th></tr>";
				}
				else
				{
				$a=1;
				$sum=0;
					$mod1 = AccountHead::find()->where(['id' => $value->account_payable])->andwhere(['organization_id' => \Yii::$app->user->identity->organization_id])->One(); 
					$returndata.='<tr class="text-center">
							<th class="text-center text-dark">'.$a.'</th>
							<th class="text-center text-dark">'.$value->transaction_id.'</th>
							<th class="text-center text-dark">'.number_format($value->amount).'</th>
							<th class="text-center text-dark">'.$mod1->account_name.'</th>
							<th class="text-center text-dark">'.$value->due_date.'</th>
							<th class="text-center text-dark">'.$value->created_at.'</th>
						</tr>';
						$sum = $sum + $value->amount;
						$returndata.='<tr style="background:grey;color:white"><th colspan="2" class="text-center">Total Account Payable</th><th class="text-center">'.number_format($sum).'</th></tr></table>';
				}
					$returndata.='<div class="row" style="margin:0px;"><div class="col-md-12"><h4 style="margin-left:10px;">Transactions Info</h4></div></div><table style="margin-top:10px !important" class="table  table-responsive tbale-bordered table-striped">
						...
						$sum1=0;
					$trans_value = Transactions::find()->where(['Between','transaction_date' , $start_date,$end_date])->andwhere(['debit_account' => $account_payable_id])->andwhere(['organization_id' => \Yii::$app->user->identity->organization_id])->all();

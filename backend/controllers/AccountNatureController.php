...
use backend\models\AccountNatureSearch;
...
class AccountNatureController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $model->created_at = date('Y-m-d h:m:s');
                ...
                $record = AccountNature::find()->orderBy(['id' => SORT_DESC])->One();

                if(empty($record))
                {
                    $append = 1;
                    $model->account_no = '0' . $append . '-000';
                }
                else
                {
                $id_value = $record->account_no;
                if((int)$id_value >=10)
                {
                    $sbtr = substr($id_value,0,3);
                    (int)$sbtr = preg_replace("/[-]/", "", $sbtr);
                }
                else
                {
                    $sbtr = substr($id_value,0,2);
                    (int)$sbtr = preg_replace("/[-0]/", "", $sbtr);
                }
               

                (string)$append = $sbtr+1;
                if((int)$id_value>=9){
                    $model->account_no = $append . '-000';
                }else{
                    $model->account_no = '0' . $append . '-000';
                }
   ...
   public function actionGetNatureId($nature_id)
    {
        $data=AccountNature::findone($nature_id);

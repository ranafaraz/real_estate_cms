...
class PlotController extends Controller
{
    ...
    public function actionPlot($plot_no,$property_id)
    {
        ...
        $mod = Plot::find()->where(['plot_no' => $plot_no,'property_id' => $property_id])->andWhere(['status' => 'Unsold'])->andWhere(['organization_id' => \Yii::$app->user->identity->organization_id])->one();
        if($countdata > 0)
        {
                echo $mod->plot_price;
        }else
    ...
    {
        $countdata = Plot::find()->where(['property_id' => $id,'status' => 'Unsold'])->andWhere(['organization_id' => \Yii::$app->user->identity->organization_id])->count();

<?php

namespace backend\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\AccountNature;

/**
 * AccountNatureSearch represents the model behind the search form about `backend\models\AccountNature`.
 */
class AccountNatureSearch extends AccountNature
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id'], 'integer'],
            [['organization_id'],'string'],
            [['name','organization_id', 'account_no', 'created_at'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = AccountNature::find();

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }
        $query->joinWith('organization');

        $query->andFilterWhere([
            'id' => $this->id,
            
            'created_at' => $this->created_at,
        ]);


        $query->andFilterWhere(['like', 'account_nature.name', $this->name])
            ->andFilterWhere(['like', 'account_no', $this->account_no])
            ->andFilterWhere(['like', 'organization.name', $this->organization_id]);

        return $dataProvider;
    }
}
